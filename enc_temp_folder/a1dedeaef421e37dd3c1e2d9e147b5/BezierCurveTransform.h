...
public:
    ...
    glm::mat4 getMatrix() const override {
        ...
        // Normalizovaný čas (0.0 - 1.0)
        float t = elapsed / duration;
        
        if (loop) {
            t = t - std::floor(t); // Cyklické opakování
        } else {
            if (t > 1.0f) t = 1.0f; // Zastavení na konci
        }
        
        // Pozice na Bézierově křivce
        glm::vec3 position = calculateBezierPoint(t);
        
        // Základní translační matice
        glm::mat4 matrix = glm::translate(glm::mat4(1.0f), position);
        
        // Pokud chceme orientovat objekt podle směru pohybu
        if (orientToDirection) {
            glm::vec3 tangent = calculateBezierTangent(t);
            if (glm::length(tangent) > 0.001f) {
                tangent = glm::normalize(tangent);
                
                // Vypočítáme rotační matici tak, aby objekt směřoval podle tečny
                // Předpokládáme, že objekt standardně směřuje ve směru +Z
                glm::vec3 defaultForward = glm::vec3(0.0f, 0.0f, 1.0f);
                
                // Normalizovaný tečný vektor je náš nový forward směr
                glm::vec3 right = glm::normalize(glm::cross(glm::vec3(0.0f, 1.0f, 0.0f), tangent));
                if (glm::length(right) < 0.001f) {
                    // Tečna je téměř vertikální, použijeme jiný up vektor
